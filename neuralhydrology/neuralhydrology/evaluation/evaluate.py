...
def ESDL_start_evaluation(cfg: Config, run_dir: Path, epoch: int = None, period: str = "test"):
    """Start evaluation of a trained network
    ...
    """
    tester = get_tester(cfg=cfg, run_dir=run_dir, period=period, init_model=True)
    #save_results set to false to avoid writing results to disk as they will get replaced every time
    return tester.evaluate(epoch=epoch, save_results=False, save_all_output=cfg.save_all_output, metrics=cfg.metrics)